...
export default function QuizManagePage() {
  ...
  return <div className="page"><TopBar /><main className="container quiz-container">
    ...
    {!data ? <p>正在加载片段…</p> : <>
      ...
      <p className="quiz-note">先分类练习类型，再筛选发球候选。默认显示发接发；不确定、连续对练和其他片段可在右侧筛选中回看。确认片段不代表确认旋转答案，练习暂不计分。</p>
      ...
      <div className="quiz-workspace"><section className="quiz-editor card">
        ...
        <PracticeContext data={data.practice} selectedId={windowId} onSelect={setWindowId} busy={busy} onReview={reviewPractice} onPreview={w => {player.current?.pause(); stopAt.current = w.end_ms / 1000; player.current?.seekTo(w.start_ms / 1000); player.current?.play();}} />
